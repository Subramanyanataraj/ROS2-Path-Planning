...
DIJKSTRA::DIJKSTRA():free_space(0), start_value(-2), goal_value(-120), path_finding_color(0.00005), 
final_color(-127),color_update(98),data_recieved(false),cost(1),first_run(true),goal_reached(false),m_height(0),m_width(0), Node("dijkstra_node")
{
...
auto result_future = map_client->async_send_request(
        request, std::bind(&DIJKSTRA::get_map_callback, this,
                           std::placeholders::_1));
...
timer = create_wall_timer(0.001s, std::bind(&DIJKSTRA::path_planning_callback, this));
...
void DIJKSTRA::get_map_callback(rclcpp::Client<nav_msgs::srv::GetMap>::SharedFuture future)
{   
  data_recieved = true;
  result = future.get();
  map_data = *result;
...
void DIJKSTRA::path_planning_callback()
{ 
  ...
  if(first_run == true)
  {
    ...
    open_heap.push_back({cost, start_x, start_y});
...
Navigating through corner obstacles
      for(int i=0; i < movements.size(); i++)
      {
        ...
        { cost++;
          ...
          open_heap.push_back({cost,neighbor_x,neighbor_y});
...
*/
 ...
 nav_msgs::msg::Path DIJKSTRA::catmull_rom_spline(const nav_msgs::msg::Path path, int interpolation_points) {
...
rclcpp::spin(std::make_shared<DIJKSTRA>());
rclcpp::shutdown();
return 0;
}
